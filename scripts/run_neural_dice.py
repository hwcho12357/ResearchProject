...
def main(argv):
  ...
  for step in range(num_steps):
    ...
    if step % 100 == 0 or step == num_steps - 1:
      estimate, weights = estimator.estimate_average_reward(dataset, target_policy)
  ...
  plt.show()
  np.savetxt('weight.txt', weights)


  plt.plot(running_estimates)
  plt.xlabel('Training Step')
  plt.ylabel('Estimated Average Reward')
  plt.title('Estimated Average Reward vs. Training Step')
